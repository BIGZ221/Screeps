 ...
 * var mod = require('role.InvaderCreeps');
...
function spawnAttackerCreep() {
    Game.spawns['Spawn1'].spawnCreep(attackerBody,"invaderAttacker".concat(Game.time % 1000),{memory:attackerCreepMemory});
}

function spawnSupportCreep() {
    Game.spawns['Spawn1'].spawnCreep(supportBody,"invaderSupport".concat(Game.time % 1000),{memory:supportCreepMemory});
}

function spawnEnergyRecoveryCreep() {
    Game.spawns['Spawn1'].spawnCreep(recoverEnergyBody,'invaderEnergyRecovery'.concat(Game.time % 1000),{memory:energyRecoveryCreepMemory});
}
...
function getEnemyDefenseStructures(currCreep, any) {
    let enemyStructs = currCreep.room.find(FIND_HOSTILE_STRUCTURES, { filter: structs => structs.structureType ===  STRUCTURE_TOWER || structs.structureType ===  STRUCTURE_RAMPART || structs.structureType === STRUCTURE_SPAWN ||any });
    return enemyStructs;
}
...
function controlAttackerCreep(currCreep) {
    if (currCreep.room !== Game.flags[currRoom].room) {
        ...
        let enemyStruct = currCreep.pos.findClosestByPath(getEnemyDefenseStructures(currCreep,false));
        let enemyCreep = currCreep.pos.findClosestByPath(getEnemyCreeps(currCreep));
...
function controlSupportCreep(currCreep) {
    let myCreeps = currCreep.room.find(FIND_MY_CREEPS, { filter: mycreeps => mycreeps.hits < mycreeps.hitsMax })
    if (currCreep.room !== Game.flags[currRoom].room) {
        if (groupedUp === false) {
            currCreep.moveTo(Game.flags[waitArea]);
        } else {
        ...
        if (myCreeps.length !== 0) {
            ...
            let enemyStructure = currCreep.pos.findClosestByPath(getEnemyDefenseStructures(currCreep,true));
...
function controlEnergyRecoveryCreep(currCreep) {
    ...
    if (currCreep.room !== Game.flags[currRoom].room && currCreep.memory.isFull === false) {
        currCreep.moveTo(Game.flags[currRoom]);
    } else if (currCreep.room === Game.flags[currRoom].room) {
        ...
        if (currCreep.room !== Game.spawns['Spawn1'].room) {
            ...
            let storage = currCreep.room.find(FIND_MY_STRUCTURES, { filter: structs => structs.structureType === STRUCTURE_STORAGE });
            if (currCreep.transfer(storage, RESOURCE_ENERGY) === ERR_NOT_IN_RANGE) {
